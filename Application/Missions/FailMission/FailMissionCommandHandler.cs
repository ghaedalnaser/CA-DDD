...
{
    internal sealed class FailMissionCommandHandler : IRequestHandler<FailMissionCommand, Result>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMissionRepository _missionRepository;
        private readonly IMoverRepository _moverRepository;

        public FailMissionCommandHandler(
        ...
        public async Task<Result> Handle(FailMissionCommand request, CancellationToken cancellationToken)
        {
            ...
            // Fail the mission
            var result = mission.FailMission(mover);
            if (result.IsFailure)
            {
